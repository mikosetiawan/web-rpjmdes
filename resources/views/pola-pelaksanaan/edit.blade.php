<x-app-layout title="Edit Pola Pelaksanaan">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Pola Pelaksanaan</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('pola-pelaksanaan.update', $polaPelaksanaan->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="waktu_pelaksanaan_id" id="waktu_pelaksanaan_id"
                                ...
                                @foreach ($waktuPelaksanaan as $wp)
                                    <option value="{{ $wp->id }}" {{ $polaPelaksanaan->waktu_pelaksanaan_id == $wp->id ? 'selected' : '' }}>
                                        {{ $wp->tanggal_mulai }} - {{ $wp->tanggal_selesai }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="pola" id="pola" value="{{ old('pola', $polaPelaksanaan->pola) }}"
                                   class="mt-1 block w-full border-gray-300 p-2 border rounded-md shadow-sm">
                            @error('pola')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
