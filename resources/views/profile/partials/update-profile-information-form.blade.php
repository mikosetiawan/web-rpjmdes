<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __("Update your account's profile information, email address, signature, and photo.") }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            ...
            <input id="signature" name="signature" type="file" class="mt-1 block w-full" accept="image/*" />
            <x-input-error class="mt-2" :messages="$errors->get('signature')" />
            @if ($user->signature)
                <div class="mt-2">
                    <img src="{{ asset('storage/' . $user->signature) }}" alt="Signature Image" class="w-40 h-20">
        ...
        <div>
            ...
            @if ($user->photo)
                <div class="mt-2">
                    <img src="{{ asset('storage/' . $user->photo) }}" alt="Profile Photo" class="w-20 h-20 rounded-full">
